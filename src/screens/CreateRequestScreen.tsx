  ...
  if (submitted) {
    return (
      <View style={[styles.successContainer, { backgroundColor: C.background }]}> 
        ...
        <Text style={[styles.successTitle, { color: C.textPrimary }]}>Request Created!</Text>
        ...
        <View style={{ width: '80%', marginTop: 24 }}>
          <AppButton
            title={type === 'urgent' ? 'View Nearby Matches' : 'Open My Requests'}
            onPress={() => (
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: C.background }]} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <LinearGradient colors={headerGradient} style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: C.textPrimary }]}>Create Request</Text>
        <Text style={[styles.headerSub, { color: C.textSecondary }]}>Post a blood request to find donors</Text>
      </LinearGradient>

      <View style={[styles.form, { backgroundColor: C.surface, borderColor: C.border }, isDarkMode ? { borderWidth: 1 } : Shadow.lg]}>
        {/* Type Selection */}
        <Text style={[styles.label, { color: C.textPrimary }]}>Request Type</Text>
        <View style={styles.typeRow}>
          {([
            { key: 'urgent' as const, icon: 'alert-circle', title: 'Urgent', sub: 'Need blood now', color: C.primary },
            { key: 'scheduled' as const, icon: 'calendar-clock', title: 'Scheduled', sub: 'Plan ahead', color: C.info },
          ]).map(t => (
            <TouchableOpacity
              key={t.key}
              style={[
                styles.typeCard,
                {
                  ...
                  backgroundColor: type === t.key ? `${t.color}14` : C.surfaceVariant,
                },
              ]}
              onPress={() => setType(t.key)}
              activeOpacity={0.85}
            >
              <Icon name={t.icon} size={28} color={type === t.key ? t.color : C.textTertiary} />
              <Text style={[styles.typeTitle, { color: type === t.key ? t.color : C.textPrimary }]}>{t.title}</Text>
              <Text style={[styles.typeSub, { color: C.textSecondary }]}>{t.sub}</Text>
            </TouchableOpacity>
          ))}
        </View>

        {/* Blood Group */}
        <Text style={[styles.label, { color: C.textPrimary }]}>Blood Group Needed</Text>
          ...
          {BLOOD_GROUPS.map(bg => (
            <TouchableOpacity
              ...
              style={[
                styles.bgChip,
                {
                  borderColor: bloodGroup === bg ? C.primary : C.border,
                  backgroundColor: bloodGroup === bg ? C.primarySurface : C.surfaceVariant,
                },
              ]}
              onPress={() => setBloodGroup(bg)}
              activeOpacity={0.85}
            >
              <Text style={[styles.bgChipText, { color: bloodGroup === bg ? C.primary : C.textSecondary }]}>{bg}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.inputGroup}>
          <Text style={[styles.label, { color: C.textPrimary }]}>Units Required</Text>
            ...
            {['1', '2', '3', '4', '5+'].map(u => (
              <TouchableOpacity
                ...
                style={[
                  styles.unitChip,
                  {
                    borderColor: units === u ? C.primary : C.border,
                    backgroundColor: units === u ? C.primarySurface : C.surfaceVariant,
                  },
                ]}
                onPress={() => setUnits(u)}
                activeOpacity={0.85}
              >
                <Text style={[styles.unitText, { color: units === u ? C.primary : C.textSecondary }]}>{u}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        {/* Notes */}
        <View style={styles.inputGroup}>
          <Text style={[styles.label, { color: C.textPrimary }]}>Additional Notes</Text>
          <View style={[styles.inputRow, { height: 80, alignItems: 'flex-start', paddingTop: 12, backgroundColor: C.surfaceVariant, borderColor: C.border }]}> 
            <Icon name="note-text" size={20} color={C.textTertiary} />
            <TextInput
              style={[styles.input, { textAlignVertical: 'top', color: C.textPrimary }]}
        ...
        <View style={{ marginTop: 8 }}>
          <AppButton
            title={type === 'urgent' ? 'Post Urgent Request' : 'Schedule Request'}
            onPress={handleSubmit}
            iconLeft={type === 'urgent' ? 'alert-circle' : 'calendar-check'}
            variant="primary"
            gradientColors={type === 'urgent' ? C.gradientPrimary : [C.info, '#1D4ED8']}
          />
        </View>
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: FontSize.xxl, fontWeight: FontWeight.extrabold, marginTop: 16 },
  ...
  typeCard: { flex: 1, padding: 16, borderRadius: BorderRadius.lg, borderWidth: 1.5, alignItems: 'center', gap: 4 },
  ...
  unitChip: { flex: 1, height: 44, borderRadius: BorderRadius.md, borderWidth: 1.5, justifyContent: 'center', alignItems: 'center' },
