...
const RewardsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
    ...
    const [leaderTab, setLeaderTab] = useState<"city" | "state" | "country">(
        "city",
    );
    const [subTab, setSubTab] = useState<"individuals" | "banks">(
        "individuals",
    );
    ...
    const LEVEL_THRESHOLDS: Record<
        string,
        { min: number; max: number; next: string }
    > = {
    ...
    const thresholds =
        LEVEL_THRESHOLDS[currentLevel] || LEVEL_THRESHOLDS.Bronze;
    const range = thresholds.max - thresholds.min;
    const levelProgress =
        range > 0
            ? Math.min(
                  Math.max(
                      Math.round(((userCoins - thresholds.min) / range) * 100),
                      0,
                  ),
                  100,
              )
    ...
    const filteredLeaderboard = (
        subTab === "individuals" ? leaderboardData : bankLeaderboard
    ).filter((entry) => {
            ...
            <LinearGradient colors={headerGradient} style={styles.header}>
                ...
                <View
                    style={[
                        styles.tokenCard,
                        {
                            backgroundColor: "rgba(88,108,226,0.1)",
                ...
                <View style={styles.levelProgress}>
                    <View style={styles.levelRow}>
                        ...
                        >
                            {currentLevel === "Platinum"
                                ? "Max level"
                                : `${coinsToNext} coins to ${nextLevel}`}
                        </Text>
                    </View>
                    <View
                        style={[
                    ...
                    {(["individuals", "banks"] as const).map((tab) => (
                        ...
                        >
                            <Icon
                                ...
                                size={16}
                                color={
                                    subTab === tab ? C.primary : C.textTertiary
                                }
                            />
                            <Text
                                style={[
