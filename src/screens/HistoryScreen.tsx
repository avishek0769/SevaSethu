...
const STATUS_CONFIG: Record<
    RequestOutcome,
    { label: string; icon: string; color: string; bg: string; note: string }
> = {
    pending: {
...
const buildTimeline = (entry: AcceptedHistoryCard): TimelineStep[] => {
    ...
    const confirmedTime =
        entry.confirmedAt ||
        (entry.status === "completed" ? entry.date : undefined);
        ...
        if (rejectedTime) {
            steps.push({
                ...
                description:
                    ...
                    "The requester thanked you for your quick response and moved ahead with a different donor.",
                time: rejectedTime,
            ...
            steps.push({
                ...
                description:
                    "The request is no longer active. Your intent to help is still recorded in history.",
                time: rejectedTime,
                icon: "heart-outline",
                tone: "info",
            });
    ...
    if (confirmedTime) {
        steps.push({
            ...
            description:
                "The donor has donated the blood and the requester confirmed it.",
            time: confirmedTime,
        ...
        steps.push({
            ...
            description:
                "Download the certificate from this card once the donation is marked complete.",
            time: confirmedTime,
...
const HistoryScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
    ...
    const acceptedEntries = useMemo<AcceptedHistoryCard[]>(() => {
        const cards = historyEntries
            .filter(
                (entry): entry is HistoryEntry & { type: "accepted" } =>
                    entry.type === "accepted",
            )
            .map((entry) => {
                const relatedRejection = historyEntries.find(
                    (other) =>
                        other.type === "rejected" &&
        ...
        return cards.sort(
            (left, right) =>
                new Date(right.acceptedAt || right.date).getTime() -
                new Date(left.acceptedAt || left.date).getTime(),
        );
    }, [historyEntries]);
    ...
    const visibleEntries =
        selectedFilter === "All"
            ...
            : acceptedEntries.filter(
                  (entry) =>
                      entry.outcome ===
                      (selectedFilter.toLowerCase() as RequestOutcome),
              );

    const totalConfirmed = acceptedEntries.filter(
        (entry) => entry.outcome === "confirmed",
    ).length;
    const totalRejected = acceptedEntries.filter(
        (entry) => entry.outcome === "rejected",
    ).length;
    const totalCoins = acceptedEntries.reduce(
        (acc, entry) => acc + (entry.tokensEarned || 0),
        0,
    );
        ...
        return (
            ...
            >
                <AppCard
                    style={[
                        styles.historyCard,
                        isDarkMode && { backgroundColor: C.darkSurface },
                    ]}
                >
                    ...
                    <Text
                        style={[styles.cardNote, { color: C.textSecondary }]}
                        numberOfLines={2}
                    >
                        {config.note}
                    </Text>
        ...
        <View style={[styles.container, { backgroundColor: bg }]}>
            ...
            <LinearGradient colors={headerGradient} style={styles.header}>
                ...
                <Text style={styles.headerSub}>
                    Tap any card to review the requester, donation status,
                                ...
                                <View style={styles.modalHeader}>
                                    <View style={styles.modalHeaderCopy}>
                                        ...
                                        <Text
                                            ...
                                            numberOfLines={1}
                                        >
                                            {selectedEntry.hospital ||
                                                "Blood request"}{" "}
                                            ·{" "}
                                            {selectedEntry.requestType ===
                                            "urgent"
                                                ? "Urgent"
                                                : "Scheduled"}
                                        </Text>
