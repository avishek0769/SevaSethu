  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: bg }]} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <LinearGradient colors={headerGradient} style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: C.textOnPrimary }]}>About SevaSethu</Text>
        <Text style={[styles.headerSub, { color: isDarkMode ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.8)' }]}>A simple blood donation ecosystem built to connect people faster.</Text>
      </LinearGradient>

      <View style={styles.body}>
        <AppCard style={styles.heroCard}>
          <Text style={[styles.heroTitle, { color: C.textPrimary }]}>Saving lives faster, one request at a time.</Text>
          <Text style={[styles.heroBody, { color: C.textSecondary }]}>SevaSethu helps donors, requesters, and blood banks coordinate urgent and scheduled blood needs in one place. The goal is to reduce friction and keep life-saving action visible.</Text>
        </AppCard>
        ...
        {[
          ...
          <AppCard key={item.title} style={styles.sectionCard}>
            <Text style={[styles.sectionTitle, { color: C.textPrimary }]}>{item.title}</Text>
            <Text style={[styles.sectionBody, { color: C.textSecondary }]}>{item.body}</Text>
          </AppCard>
        ))}
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: FontSize.xxl, fontWeight: FontWeight.extrabold, marginTop: 14 },
  headerSub: { fontSize: FontSize.sm, marginTop: 4 },
  body: { padding: 20 },
  heroCard: { marginBottom: 12 },
  heroTitle: { fontSize: FontSize.xl, fontWeight: FontWeight.bold, marginBottom: 8 },
  heroBody: { fontSize: FontSize.md, lineHeight: 22 },
  sectionCard: { marginBottom: 12 },
  sectionTitle: { fontSize: FontSize.lg, fontWeight: FontWeight.bold, marginBottom: 8 },
