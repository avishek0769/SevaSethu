    ...
    <ScrollView style={[styles.container, { backgroundColor: bg }]} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.form}>
        ...
        <View style={styles.inputGroup}>
          <Text style={[styles.label, { color: C.textPrimary }]}>Age</Text>
          <View style={[styles.inputRow, { backgroundColor: C.surfaceVariant, borderColor: C.border }]}>
            <Icon name="calendar" size={20} color={C.textTertiary} />
            <TextInput style={[styles.input, { color: C.textPrimary }]} placeholder="Enter your age" placeholderTextColor={C.textTertiary} value={age} onChangeText={setAge} keyboardType="number-pad" />
          </View>
        </View>
        ...
        <View style={styles.genderRow}>
          {GENDERS.map(option => (
            <TouchableOpacity key={option} style={[styles.genderChip, gender === option && styles.genderChipActive]} onPress={() => setGender(option)}>
              <Icon name={gender === 'Male' ? 'gender-male' : gender === 'Female' ? 'gender-female' : 'gender-non-binary'} size={18} color={gender === option ? C.primary : C.textTertiary} />
              <Text style={[styles.genderText, gender === option && { color: C.primary }]}>{option}</Text>
            </TouchableOpacity>
          ))}
        </View>
...
const styles = StyleSheet.create({
  ...
  genderChip: { flex: 1, height: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: BorderRadius.md, borderWidth: 1.5, backgroundColor: Colors.surfaceVariant },
