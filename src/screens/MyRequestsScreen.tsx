...
const MyRequestsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
    ...
    const [activeTab, setActiveTab] = useState<"urgent" | "scheduled">(
        "urgent",
    );
    ...
    const myUrgentRequests = urgentRequests.filter(
        (request) => request.requesterId === user.id,
    );
    const myScheduledRequests = scheduledRequests.filter(
        (request) => request.requesterId === user.id,
    );
    const showcaseUrgentRequests = urgentRequests.slice(0, 2);
    const showcaseScheduledRequests = scheduledRequests.slice(0, 2);
    const visibleUrgentRequests =
        myUrgentRequests.length > 0 ? myUrgentRequests : showcaseUrgentRequests;
    const visibleScheduledRequests =
        myScheduledRequests.length > 0
            ? myScheduledRequests
            : showcaseScheduledRequests;
    const activeRequests =
        activeTab === "urgent"
            ? visibleUrgentRequests
            : visibleScheduledRequests;

    const acceptedCount = [
        ...visibleUrgentRequests,
        ...visibleScheduledRequests,
    ].reduce((sum, request) => sum + (request.acceptedDonors?.length || 0), 0);
    const confirmedCount = [
        ...
        (sum, request) =>
            sum +
            (request.acceptedDonors || []).filter((donor) => donor.confirmed)
                .length,
        0,
    );
    ...
    const renderRequestCard = (
        request:
            | (typeof myUrgentRequests)[number]
            | (typeof myScheduledRequests)[number],
        requestType: "urgent" | "scheduled",
    ) => {
        const accepted = request.acceptedDonors?.length || 0;
        const confirmed = (request.acceptedDonors || []).filter(
            (donor) => donor.confirmed,
        ).length;
        ...
        return (
            <AppCard
                key={request.id}
                style={styles.card}
                onPress={() => openRequestDetails(requestType, request.id)}
            >
                <View style={styles.cardHeader}>
                    ...
                    {requestType === "scheduled" ? (
                        ...
                        >
                            ...
                            <Text
                                style={[styles.dateChipText, { color: C.info }]}
                            >
                    ...
                    <TouchableOpacity
                        style={[
                            styles.dateChip,
                            { backgroundColor: C.errorLight, marginLeft: 8 },
                        ]}
                        onPress={(e) => {
                            e.stopPropagation();
                    ...
                    >
                        ...
                        <Text style={[styles.dateChipText, { color: C.error }]}>
                            Close
                        </Text>
                    </TouchableOpacity>
                </View>
                ...
                {request.notes ? (
                    <Text
                        style={[styles.notes, { color: C.textSecondary }]}
                        numberOfLines={2}
                    >
                        {request.notes}
                    </Text>
                ...
                <View style={styles.footerRow}>
                    ...
                    >
                        <Icon
                            ...
                            color={C.success}
                        />
                        <Text style={[styles.countText, { color: C.success }]}>
                            {confirmed} confirmed
                        </Text>
                    </View>
                </View>
        ...
        >
            ...
            <LinearGradient colors={headerGradient} style={styles.header}>
                ...
                <Text style={[styles.headerSub, { color: C.textSecondary }]}>
                    Keep track of every request you create and who responds.
                </Text>
                <View style={styles.summaryRow}>
                    ...
                    >
                        ...
                        >
                            {myUrgentRequests.length +
                                myScheduledRequests.length}
                        </Text>
                        <Text
                            style={[
                                styles.summaryLabel,
            ...
            <View style={styles.filterRow}>
                {(["urgent", "scheduled"] as const).map((tab) => {
                    ...
                    return (
                        <FilterChip
                            key={tab}
                            label={`${tab === "urgent" ? "Urgent" : "Scheduled"} (${count})`}
            ...
            >
                <LinearGradient
                    ...
                    style={[styles.fabInner, !isDarkMode && Shadow.red]}
                >
