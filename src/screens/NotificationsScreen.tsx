...
import { Colors, getColors, FontSize, FontWeight, BorderRadius } from '../utils/theme';
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: bg }]} showsVerticalScrollIndicator={false}>
      ...
      <LinearGradient colors={headerGradient} style={styles.header}>
        ...
        {unreadCount > 0 && (
          <View style={[styles.unreadBadge, { backgroundColor: C.primarySurface }]}> 
            <Text style={[styles.unreadText, { color: C.primary }]}>{unreadCount} new</Text>
          </View>
        )}
      </LinearGradient>
        ...
        {notifications.map(notif => {
          ...
          return (
            <TouchableOpacity key={notif.id} activeOpacity={0.7} onPress={() => markNotificationRead(notif.id)}>
              <AppCard style={!notif.isRead ? [styles.notifCard, { borderLeftColor: C.primary }, styles.unreadCard] : styles.notifCard}>
                <View style={styles.notifRow}>
                  ...
                  <View style={styles.notifContent}>
                    <View style={styles.notifTop}>
                      <Text style={[styles.notifTitle, { color: C.textPrimary }]} numberOfLines={1}>{notif.title}</Text>
                      {!notif.isRead && <View style={[styles.unreadDot, { backgroundColor: C.primary }]} />}
                    </View>
                    <Text style={[styles.notifMessage, { color: C.textSecondary }]} numberOfLines={2}>{notif.message}</Text>
                    <Text style={[styles.notifTime, { color: C.textTertiary }]}>{notif.timestamp}</Text>
                  </View>
                </View>
              </AppCard>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { flex: 1, fontSize: FontSize.xxl, fontWeight: FontWeight.extrabold },
  unreadBadge: { paddingHorizontal: 12, paddingVertical: 4, borderRadius: BorderRadius.full },
