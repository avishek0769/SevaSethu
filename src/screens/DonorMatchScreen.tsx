...
const DonorMatchScreen: React.FC<{ navigation: any; route: any }> = ({
    navigation,
    route,
}) => {
    const { isDarkMode, bloodBanks, fetchMatchedDonors, fetchBloodBanks } =
        useApp();
    ...
    const matchingBanks = bloodBanks.filter((bank) =>
        bank.availableGroups.includes(bloodGroup),
    );
    ...
    const notifyDonor = (name: string) => {
        Alert.alert(
            "Request sent",
            `${name} has been notified for this ${requestType} request.`,
        );
    };
        ...
        >
            ...
            >
                ...
                <Text style={styles.headerSub}>
                    {matchingDonors.length} donors · {matchingBanks.length}{" "}
                    banks for {bloodGroup}
                </Text>
            </LinearGradient>
            ...
            <View style={styles.infoBar}>
                ...
                <Text style={styles.infoText}>
                    Donors and banks are filtered by the request blood group and
                    can be revisited later from My Requests.
                </Text>
            </View>
                    ...
                    <AppCard key={donor.id} style={styles.donorCard}>
                        ...
                        <View style={styles.actionRow}>
                            ...
                            <TouchableOpacity
                                style={{ flex: 1 }}
                                activeOpacity={0.8}
                                onPress={() => notifyDonor(donor.name)}
                            >
                                <LinearGradient
                                    colors={["#DC2626", "#991B1B"]}
                    ...
                    <AppCard key={bank.id} style={styles.bankCard}>
                        ...
                        <View style={styles.actionRow}>
                            <TouchableOpacity
                                style={styles.callBtn}
                                onPress={() =>
                                    Linking.openURL(`tel:${bank.phone}`)
                                }
                                activeOpacity={0.7}
                            >
                                <Icon
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() =>
                                    Alert.alert(
                                        "Blood bank notified",
                                        `${bank.name} has been contacted for this request.`,
                                    )
                                }
                            >
                                <LinearGradient
