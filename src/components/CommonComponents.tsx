...
import { Colors, getColors, Spacing, BorderRadius, FontSize, FontWeight, Shadow } from '../utils/theme';
...
export const BloodGroupBadge: React.FC<BloodGroupBadgeProps> = ({ bloodGroup, size = 'md' }) => {
  ...
  const s = sizes[size];
  return (
    <LinearGradient colors={Colors.gradientPrimary} style={[styles.bloodBadge, { width: s.w, height: s.h, borderRadius: s.w / 2 }]}>
...
export const AppButton: React.FC<AppButtonProps> = ({
  ...
  const finalGradient = gradientColors?.length ? gradientColors : C.gradientPrimary;
  const isDisabled = Boolean(disabled || loading);

  const baseContent = (
    <View style={styles.appBtnContent}>
      {loading ? (
        <ActivityIndicator size="small" color={variant === 'primary' ? C.textOnPrimary : finalAccent} />
      ) : iconLeft ? (
        <Icon
          name={iconLeft}
          size={fontSizes[size] + 3}
          color={variant === 'primary' ? C.textOnPrimary : finalAccent}
          style={{ marginRight: 8 }}
        />
      ) : null}
      ...
      <Text
        style={[
          styles.appBtnText,
          {
            ...
            color: variant === 'primary' ? C.textOnPrimary : (variant === 'secondary' ? C.textPrimary : finalAccent),
          },
          isDisabled && variant !== 'primary' && { color: C.textTertiary },
          textStyle,
        ]}
      >
        {title}
  ...
  if (variant === 'primary') {
    return (
      <Pressable onPress={onPress} disabled={isDisabled} style={pressedStyle}>
        <LinearGradient
          colors={isDisabled ? [C.textTertiary, C.textSecondary] : finalGradient}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 0 }}
          style={[styles.appBtnBase, { height: heights[size] }, !isDarkMode && Shadow.red]}
        >
          {baseContent}
        </LinearGradient>
      </Pressable>
  ...
  return (
    <Pressable onPress={onPress} disabled={isDisabled} style={pressedStyle}>
      <View style={[styles.appBtnBase, { height: heights[size] }, variantStyle]}>{baseContent}</View>
    </Pressable>
  );
};
...
export const AppTextField: React.FC<AppTextFieldProps> = ({
  ...
  return (
    <View style={containerStyle}>
      {label ? <Text style={[styles.fieldLabel, { color: C.textPrimary }]}>{label}</Text> : null}
      <View style={[styles.fieldRow, { borderColor, backgroundColor }]}>
        {iconLeft ? <Icon name={iconLeft} size={20} color={C.textTertiary} /> : null}
        <TextInput
        ...
        />
        {iconRight ? (
          <TouchableOpacity onPress={onPressIconRight} activeOpacity={0.8}>
            <Icon name={iconRight} size={20} color={C.textTertiary} />
          </TouchableOpacity>
        ) : null}
      </View>
      {errorText ? (
        <Text style={[styles.fieldHint, { color: C.error }]}>{errorText}</Text>
      ) : helperText ? (
        <Text style={[styles.fieldHint, { color: C.textSecondary }]}>{helperText}</Text>
      ) : null}
    </View>
  );
};
...
// ============ AppBadge ==========
type AppBadgeTone = 'neutral' | 'primary' | 'success' | 'warning' | 'info' | 'danger';
...
export const AppBadge: React.FC<AppBadgeProps> = ({ label, tone = 'neutral', icon, style }) => {
  const { isDarkMode } = useApp();
  const C = getColors(isDarkMode);
...
export const GradientButton: React.FC<GradientButtonProps> = ({
  title, onPress, icon, colors = Colors.gradientPrimary, style, textStyle, disabled, size = 'md',
}) => {
  return (
    <AppButton
      title={title}
...
export const OutlineButton: React.FC<OutlineButtonProps> = ({ title, onPress, icon, color = Colors.primary, style, disabled }) => (
  <AppButton
    title={title}
    onPress={onPress}
    iconLeft={icon}
    ...
    <View style={styles.sectionHeader}>
      ...
      {actionText && (
        <TouchableOpacity onPress={onAction}>
          <Text style={[styles.sectionAction, { color: C.primary }]}>{actionText}</Text>
        </TouchableOpacity>
      )}
    </View>
  );
...
export const EmptyState: React.FC<EmptyStateProps> = ({ icon, title, message, actionText, onAction }) => {
  ...
  return (
    <View style={styles.emptyState}>
      ...
      <Text style={[styles.emptyMessage, { color: C.textSecondary }]}>{message}</Text>
...
export const StatCard: React.FC<StatCardProps> = ({ icon, label, value, color = Colors.primary }) => {
  ...
  const C = getColors(isDarkMode);
  return (
    <View style={[styles.statCard, { backgroundColor: C.surface }, isDarkMode ? { borderColor: C.border, borderWidth: 1 } : Shadow.sm]}>
...
export const SearchBar: React.FC<SearchBarProps> = ({ placeholder = 'Search...', value, onChangeText }) => {
  ...
  const C = getColors(isDarkMode);
  return (
    <View style={[styles.searchBar, { backgroundColor: C.surfaceVariant, borderColor: C.border }]}>
...
const FilterChipInner: React.FC<FilterChipProps> = ({ label, isSelected, onPress }) => {
  const { isDarkMode } = useApp();
  const C = getColors(isDarkMode);
  ...
  return (
    ...
    >
      <Text style={[styles.filterChipText, { color: textColor }, isSelected && styles.filterChipTextActive]}>{label}</Text>
    </Pressable>
  );
};
...
export const ConfirmationDialog: React.FC<ConfirmationDialogProps> = ({
  ...
  return (
    <Modal transparent visible={visible} animationType="fade" onRequestClose={onCancel}>
      ...
      <Pressable style={[styles.dialogCard, { backgroundColor: cardBackground, borderColor: C.border }]} onPress={() => {}}>
            ...
            {showCancel ? (
              <TouchableOpacity
                style={[
                  ...
                  {
                    borderColor: C.border,
                    backgroundColor: isDarkMode ? C.surfaceVariant : C.white,
                  },
                ]}
                onPress={onCancel}
                activeOpacity={0.8}
...
// ============ SkeletonLoader ============
export const SkeletonLoader: React.FC<{ width?: number | string; height?: number; style?: StyleProp<ViewStyle> }> = ({ width = '100%', height = 16, style }) => {
  const { isDarkMode } = useApp();
  const C = getColors(isDarkMode);
...
const styles = StyleSheet.create({
  ...
  appBtnBase: { borderRadius: BorderRadius.md, paddingHorizontal: Spacing.xl, justifyContent: 'center', alignItems: 'center', overflow: 'hidden' },
  appBtnContent: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
  ...
  emptyIconCircle: { width: 96, height: 96, borderRadius: 48, justifyContent: 'center', alignItems: 'center', marginBottom: Spacing.lg },
  emptyTitle: { fontSize: FontSize.xl, fontWeight: FontWeight.bold, marginBottom: Spacing.sm },
  emptyMessage: { fontSize: FontSize.md, textAlign: 'center', lineHeight: 22 },
  ...
  searchBar: { flexDirection: 'row', alignItems: 'center', borderRadius: BorderRadius.md, paddingHorizontal: Spacing.lg, paddingVertical: Spacing.md, marginHorizontal: Spacing.xl, marginBottom: Spacing.md, borderWidth: 1 },
  searchInput: { flex: 1, marginLeft: Spacing.sm, fontSize: FontSize.md, paddingVertical: 0 },
  filterChip: { minHeight: 40, paddingHorizontal: Spacing.lg, paddingVertical: 10, borderRadius: BorderRadius.full, marginRight: Spacing.sm, borderWidth: 1, justifyContent: 'center' },
  ...
  dialogOverlay: { flex: 1, backgroundColor: Colors.overlay, justifyContent: 'center', padding: 24 },
  ...
  fieldLabel: { fontSize: FontSize.md, fontWeight: FontWeight.semibold, marginBottom: 8 },
  fieldRow: { flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderRadius: BorderRadius.md, paddingHorizontal: 16, minHeight: 52 },
  fieldInput: { flex: 1, marginLeft: 12, fontSize: FontSize.md, paddingVertical: 0 },
  fieldHint: { marginTop: 6, fontSize: FontSize.sm, lineHeight: 18 },
  badge: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 6, borderRadius: BorderRadius.full, maxWidth: '100%' },
