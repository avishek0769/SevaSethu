...
import type { Alert, BloodBank, Donation, LeaderboardEntry, Request, User } from "../data/types";
type State = {
  theme: ThemeMode;
  users: User[];
  requests: Request[];
...
const initialTheme: ThemeMode = typeof window !== "undefined" ? getInitialTheme() : "light";
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "users/verify":
      return {
        ...state,
        users: state.users.map((u) => (u.id === action.id ? { ...u, isVerified: action.value ?? !u.isVerified } : u)),
      };

    case "users/flag":
      return {
        ...state,
        users: state.users.map((u) => (u.id === action.id ? { ...u, isFlagged: action.value ?? !u.isFlagged } : u)),
      };
    ...
    case "donations/setStatus":
      return {
        ...state,
        donations: state.donations.map((d) => (d.id === action.id ? { ...d, status: action.status } : d)),
      };

    case "donations/adjustTokens":
      return {
        ...state,
        donations: state.donations.map((d) => (d.id === action.id ? { ...d, tokens: action.tokens } : d)),
      };

    case "alerts/markNotified":
      return {
        ...state,
        alerts: state.alerts.map((a) => (a.id === action.id ? { ...a, notified: true } : a)),
      };
    ...
    case "bloodBanks/update":
      return {
        ...state,
        bloodBanks: state.bloodBanks.map((b) => (b.id === action.bank.id ? action.bank : b)),
      };
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const actions = useMemo<Store["actions"]>(
    () => ({
      ...
      verifyUser: (id, value) => dispatch({ type: "users/verify", id, value }),
      flagUser: (id, value) => dispatch({ type: "users/flag", id, value }),
      setRequestStatus: (id, status) => dispatch({ type: "requests/setStatus", id, status }),
      setDonationStatus: (id, status) => dispatch({ type: "donations/setStatus", id, status }),
      adjustDonationTokens: (id, tokens) => dispatch({ type: "donations/adjustTokens", id, tokens }),
      markAlertNotified: (id) => dispatch({ type: "alerts/markNotified", id }),
      createBloodBank: (bank) => dispatch({ type: "bloodBanks/create", bank }),
      updateBloodBank: (bank) => dispatch({ type: "bloodBanks/update", bank }),
      deleteBloodBank: (id) => dispatch({ type: "bloodBanks/delete", id }),
    }),
    []
  );
