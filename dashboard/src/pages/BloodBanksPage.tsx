...
export function BloodBanksPage() {
  ...
  const editing = useMemo(
    () => (editingId ? state.bloodBanks.find((b) => b.id === editingId) ?? null : null),
  ...
  const submit = () => {
    if (!form.name.trim() || !form.address.trim() || !form.contact.trim() || !form.city.trim() || !form.state.trim()) {
  ...
  return (
    <div className="grid gap-6">
      <Card>
        <CardHeader>
          <div>
            ...
            <CardMeta>Register + manage blood banks (mock)</CardMeta>
          </div>
      ...
      <Card>
        <CardHeader>
          <div>
            <CardTitle>{editing ? "Edit Blood Bank" : "Register Blood Bank"}</CardTitle>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <Input value={form.contact} onChange={(e) => setForm((p) => ({ ...p, contact: e.target.value }))} placeholder="Contact" />
        ...
        <div className="mt-4">
          <div className="text-xs font-semibold text-[var(--fg-2)]">Supported Blood Groups</div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setForm((p) => {
                        ...
                        if (e.target.checked) next.add(bg);
                        else next.delete(bg);
                        return { ...p, groups: Array.from(next) as any };
                      });
                    }}
                  />
                  {bg}
      ...
      <Table>
        ...
        <tbody>
          {state.bloodBanks.map((b) => (
            <tr key={b.id} className="hover:bg-[var(--surface-2)]/60">
              <Td>
                <div className="font-semibold">{b.name}</div>
                <div className="text-xs text-[var(--fg-2)]">{b.contact}</div>
              </Td>
              <Td>
                <div className="font-medium">{b.city}</div>
                <div className="text-xs text-[var(--fg-2)]">{b.state}</div>
              </Td>
              <Td>
              ...
              <Td>
                <Badge tone={b.status === "active" ? "success" : "neutral"} className="capitalize">
                  {b.status}
                </Badge>
              </Td>
              <Td className="text-right">
                <div className="flex justify-end gap-2">
                  <Button variant="secondary" size="sm" onClick={() => setEditingId(b.id)}>
                    Edit
                  </Button>
                  <Button variant="danger" size="sm" onClick={() => actions.deleteBloodBank(b.id)}>
                    Delete
                  </Button>
                </div>
              </Td>
